...
window.addEventListener("pointermove", (e) => {
  ...
  if (drag.type === "create") {
    ...
    let ns = Math.max(Math.min(0, 15 - dur), Math.min(1440 - 15, drag.cs + dm));
    drag.dm = ns - drag.cs;
...
$("fileIn").addEventListener("change", async () => {
  ...
  try {
    ...
    save();
    render();
    undoable(`Imported ${state.title} (${state.events.length} events)`, () => {
      state = prev;
      save();
      render();
    });
...
$("sStart").addEventListener("change", () => {
  ...
  if (v >= state.settings.endHour) state.settings.endHour = Math.min(24, v + 1);
  setS("startHour", v);
});
...
$("sLen").addEventListener("change", () => setS("clickLen", +$("sLen").value));
...
$("sSample").addEventListener("click", async () => {
  ...
  try {
    ...
    toast(
      "Couldn't load the sample week. Check that " + SAMPLE_URL + " is there.",
    );
    return;
  }
